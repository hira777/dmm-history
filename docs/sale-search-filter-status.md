...
- フェーズ 8: **確認** - 自動確認と手動確認を行う
  - [x] `pnpm format:check` を実行する
  - [x] `pnpm check-types` を実行する
  - [x] `pnpm test` を実行する
  - [x] `pnpm build` を実行する
  - [x] 実サイトで検索窓付近にプルダウンが表示されることを確認する
  - [x] 「すべてのセール」で検索条件が維持されることを確認する
  - [x] 特定セールで検索条件が維持されることを確認する
  - [x] 終了したセール条件が「指定なし」に戻ることを確認する
